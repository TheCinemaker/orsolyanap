import React from 'react';
import { useOrsolya } from '../context/OrsolyaContext';
import { Flame, Clock, Plus, Check, Heart } from 'lucide-react';

export default function MenuItemCard({ item, exhibitor }) {
  const { addToCart, cart } = useOrsolya();

  const cartItem = cart.find((c) => c.item.id === item.id);
  const inCartQty = cartItem ? cartItem.quantity : 0;

  const isSoldOut = item.stock <= 0 || item.status === 'sold_out';
  const isCooking = item.status === 'cooking';

  return (
    <div className="bg-white border border-stone-200/90 rounded-2xl p-4.5 transition-all hover:border-amber-600/40 hover:shadow-md flex flex-col justify-between group">
      <div>
        {/* Header & Badges */}
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3 className="text-sm font-bold text-stone-900 group-hover:text-amber-800 transition-colors">
            {item.name}
          </h3>

          {/* Portion/Status Badge */}
          {isSoldOut ? (
            <span className="bg-stone-100 text-stone-400 text-[10px] font-bold px-2.5 py-0.5 rounded-full flex-shrink-0">
              Elfogyott
            </span>
          ) : isCooking ? (
            <span className="bg-amber-100/80 text-amber-900 text-[10px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1 flex-shrink-0 border border-amber-300/60">
              <Clock className="w-3 h-3 animate-spin text-amber-700" />
              Főzés alatt (~{item.eta_minutes || 15}p)
            </span>
          ) : (
            <span className="bg-emerald-50 text-emerald-800 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full flex-shrink-0 border border-emerald-200">
              🔥 {item.stock} adag
            </span>
          )}
        </div>

        {/* Description */}
        <p className="text-xs text-stone-600 leading-relaxed line-clamp-2">
          {item.description}
        </p>

        {/* Tags */}
        {item.tags && item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-3">
            {item.tags.map((tag, idx) => (
              <span
                key={idx}
                className="bg-stone-100 text-stone-600 text-[10px] font-medium px-2 py-0.5 rounded-md"
              >
                {tag}
              </span>
            ))}
          </div>
        )}
      </div>

      {/* Footer / Donation info & Action */}
      <div className="mt-4 pt-3 border-t border-stone-100 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1 text-[11px] font-semibold text-rose-900">
          <Heart className="w-3 h-3 text-rose-700 fill-rose-700" />
          <span>Adományos kóstolás</span>
        </div>

        <button
          onClick={() => addToCart(item)}
          disabled={isSoldOut}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
            isSoldOut
              ? 'bg-stone-100 text-stone-400 cursor-not-allowed'
              : inCartQty > 0
              ? 'bg-emerald-700 text-white shadow-xs'
              : 'bg-stone-900 hover:bg-stone-800 text-white shadow-xs'
          }`}
        >
          {inCartQty > 0 ? (
            <>
              <Check className="w-3.5 h-3.5" />
              <span>{inCartQty} adag foglalva</span>
            </>
          ) : (
            <>
              <Plus className="w-3.5 h-3.5" />
              <span>Kóstoló foglalása</span>
            </>
          )}
        </button>
      </div>
    </div>
  );
}
