...
export default function MyOrdersModal() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'ready':
        return (
          <span className="bg-emerald-700 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-xs animate-pulse">
            <Flame className="w-3.5 h-3.5 text-white" />
            <span>Átvehető a standnál!</span>
          </span>
        );
      case 'completed':
        return (
          <span className="bg-stone-100 text-stone-500 text-xs font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-xs text-stone-500 pt-1">
                    <span>
                      Átvétel: <strong className="text-amber-800">{order.pickup_time}</strong>
                    </span>
                  </div>
                </div>
              );
